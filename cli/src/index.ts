...
const main = async () => {
  ...
  try {
    ...
    try {
      ...
      throw new FileSystemError('Failed to update package.json', {
        originalError: err,
    ...
    if (!cliArgs.ci && config.install_packages) {
      ...
      try {
        ...
        throw new PackageError('Failed to install packages', {
          command,
          originalError: err,
        })
      }
      ...
      try {
        ...
        await execa(
          ...
          [
            'commit',
            '-m',
            'Initial Scaffolding from create-electron-foundation',
          ],
          {
        ...
        logger.error(
          new UnexpectedError('Failed to initialize Git', {
            projectDir: config.project_dir,
            originalError: err,
          })
        )
    ...
    if (config.ide && !cliArgs.ci) {
      ...
      try {
        ...
        logger.error(
          new UnexpectedError(`Failed to open IDE: ${config.ide}`, {
            command,
            originalError: err,
          })
        )
    ...
    logger.handleError(error, 'Main CLI process')
    process.exit(1)
  }
}
