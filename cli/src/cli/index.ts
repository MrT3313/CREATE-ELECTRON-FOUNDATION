...
export const runUserPromptCli = async (cliArgs: CLIArgs = {}): Promise<CLIResults> => {
  ...
  try {
    ...
    const initializeDatabase = cliArgs.skipPrompts ? true : ((group as any).initializeDatabase ?? false);
    const database = cliArgs.database || (initializeDatabase ? ((group as any).database || "sqlite") : null);
    const initializeORM = cliArgs.skipPrompts ? true : ((group as any).initializeORM ?? false);
    const orm = cliArgs.orm || (initializeDatabase && initializeORM ? ((group as any).orm || "drizzle") : null);
    const useTailwind = cliArgs.styles === "tailwind" || (cliArgs.styles === undefined && (cliArgs.skipPrompts ? true : ((group as any).useTailwind ?? true)));
    const initializeGit = cliArgs.initializeGit !== undefined ? cliArgs.initializeGit : (cliArgs.skipPrompts ? false : ((group as any).initializeGit ?? false));
    const installDependencies = cliArgs.installDependencies !== undefined ? cliArgs.installDependencies : (cliArgs.skipPrompts ? true : ((group as any).installDependencies ?? true));
    const runMigrations = cliArgs.runMigrations !== undefined ? cliArgs.runMigrations : (cliArgs.skipPrompts ? true : ((group as any).runMigrations ?? true));
