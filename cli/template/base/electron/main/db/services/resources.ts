...
export class resourceServices {
  static async getResourceById(id: string) {
    try {
      ...
      const info = await db.select().from(resources).where(eq(resources.id, numericId))
  ...
  static async updateResourceById(id: number, data: any) {
    try {
      ...
      if (!result || result.changes === 0) {
        return response.error({msg: 'Resource update failed - no rows affected'});
      }
      
      return response.ok();
    } catch (error) {
      return response.error({msg: `Error updating resource: ${error.message}`});
    }
  }
  ...
  static async insertResource(data: any) {
    try {
      ...
      return response.error({msg: `Error inserting resource: ${error.message}`});
    }
  }
  ...
  static async getResourceList() {
    try {
      ...
      return response.error({msg: `Error getting resource list: ${error.message}`})
    }
  }
  ...
  static async deleteResourceById(id: string) {
    try {
      ...
      if (!result || result.changes === 0) {
        return response.error({msg: 'Resource deletion failed - no rows affected'});
      }
      
      return response.ok();
    } catch (error) {
      return response.error({msg: `Error deleting resource: ${error.message}`});
    }
  }
}
